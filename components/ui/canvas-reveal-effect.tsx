...
const ShaderMaterial = ({ source, uniforms, maxFps = 60 }: { source: string; maxFps?: number; uniforms: Uniforms }) => {
  ...
  const material = useMemo(() => {
    ...
    for (const name in uniforms) {
      ...
      else if (u.type === "uniform3fv") preparedUniforms[name] = { value: (u.value as number[][]).map((v) => new THREE.Vector3().fromArray(v)) };
    }
    ...
    return new THREE.ShaderMaterial({
      vertexShader: `precision mediump float; uniform vec2 u_resolution; out vec2 fragCoord;
        void main() { gl_Position = vec4(position.xy, 0.0, 1.0); fragCoord = (position.xy + vec2(1.0)) * 0.5 * u_resolution; fragCoord.y = u_resolution.y - fragCoord.y; }`,
      fragmentShader: source,
      uniforms: preparedUniforms as Record<string, THREE.IUniform>,
